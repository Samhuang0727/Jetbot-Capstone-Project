...
def find_line(binary_warped):
    ...
    if len(leftx) > 2000:
        if len(rightx) > 2000:
            ...
            loc_mid, end_mid, endpt = draw_area(left_fit, right_fit)
            #print("1")
        else:
            left_fit = np.polyfit(lefty, leftx, 2)
            virtual_pt = np.array([[binary_warped.shape[1], 0], [binary_warped.shape[1], binary_warped.shape[0]]],
                                  dtype=np.int32)
            loc_mid, end_mid, endpt = draw_line(left_fit, virtual_pt)
            #print("2")
    else:
        if len(rightx) > 2000:
            right_fit = np.polyfit(righty, rightx, 2)
            virtual_pt = np.array([[0, 0], [0, binary_warped.shape[0]]], dtype=np.int32)
            loc_mid, end_mid, endpt = draw_line(right_fit, virtual_pt)
            #print("3")
        else:
            loc_mid = np.array([320, 479])
            end_mid = np.array([320, 0])
            endpt=np.array([320,240])
            #print("4")
    
    return loc_mid, end_mid, endpt
...
def draw_area(left_fit, right_fit):
    ...
    loc_mid=np.mean(locpts,0,dtype=np.int32)
    #cv2.line(img, loc_mid, end_mid, (255, 0, 0), 3)
    return loc_mid,end_mid,leftpte
...
def show_camera():
    ...
    if cap.isOpened():
        ...
        while 1:
            ...
            loc_mid, end_mid, endpt = find_line(pp_img)
            ...
            #cv2.circle(loc_line, (int(endpt[0][0]),int(endpt[0][1])), radius=5, color=(0, 0, 255), thickness=-1)
